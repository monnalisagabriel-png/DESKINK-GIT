...
export const BookingRequests: React.FC<BookingRequestsProps> = ({ appointments, onUpdate }) => {
    ...
    const handleAction = async (id: string, action: 'CONFIRM' | 'REJECT') => {
        setConfirmationModal({
            isOpen: true,
            title: action === 'CONFIRM' ? 'Accetta Richiesta' : 'Rifiuta Richiesta',
            message: action === 'CONFIRM'
                ? 'Vuoi accettare questa richiesta di appuntamento?'
                : 'Vuoi rifiutare questa richiesta? L\'acconto verrà rimborsato automaticamente.',
            ...
            onConfirm: async () => {
                ...
                try {
                    ...
                    if (action === 'REJECT') {
                        // 1. Trigger Stripe Refund (if payment exists)
                        try {
                            console.log('Attempting refund for:', id);
                            ...
                            if (refundError) {
                                ...
                                throw new Error("Errore durante il rimborso: " + (refundError.message || JSON.stringify(refundError)));
                            }
                            console.log('Refund result:', refundData);

                        } catch (refundErr) {
                            console.error('Critical Refund Error:', refundErr);
                            toastError("Attenzione: Il rimborso automatico ha restituito un errore.");
                        }
                        ...
                        success('Richiesta rifiutata e rimborso avviato.');
                        ...
                        success('Richiesta accettata con successo!');
                    ...
                    toastError('Errore durante l\'aggiornamento della richiesta.');
                } finally {
                    setProcessing(null);
                }
            }
